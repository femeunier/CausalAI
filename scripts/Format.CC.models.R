...
models <- rev(TrENDY.analyses::get.model.names.TRENDY("v13"))
models <- "LPJwsl"
vars <- c("gpp","nbp","ra","rh","npp","nep","nee")
...
for (imodel in seq(1,length(models))){
  ...
  r.all <- terra::mask(project(r.model.rspld,crs(raster.grid)),
                       land.frac.msk)
  ...
  for (cvar in vars){
    ...
    pos.lyrs <- which(names(r.all) == cvar)
    if (length(pos.lyrs)==0) next()
    ...
    writeRaster(r,
                file.path(cdir,paste0(cvar,".",cmodel,".tif")),
                overwrite = TRUE,
                wopt = list(gdal = "COMPRESS=LZW", datatype = "FLT4S"))

    writeRaster(r - anoms,
                file.path(cdir,paste0(cvar,"trends.",cmodel,".tif")),
                overwrite = TRUE,
                wopt = list(gdal = "COMPRESS=LZW", datatype = "FLT4S"))

    writeRaster(r_detr,
                file.path(cdir,paste0(cvar,"detrended.",cmodel,".tif")),
                overwrite = TRUE,
                wopt = list(gdal = "COMPRESS=LZW", datatype = "FLT4S"))

    writeRaster(anoms,
                file.path(cdir,paste0(cvar,"anomaly.",cmodel,".tif")),
                overwrite = TRUE,
                wopt = list(gdal = "COMPRESS=LZW", datatype = "FLT4S"))

    writeRaster(r - clim_all,
                file.path(cdir,paste0(cvar,"anomalytrends.",cmodel,".tif")),
                overwrite = TRUE,
                wopt = list(gdal = "COMPRESS=LZW", datatype = "FLT4S"))
  }
}

# scp /home/femeunier/Documents/projects/CausalAI/scripts/Format.CC.models.R hpc:/kyukon/data/gent/vo/000/gvo00074/felicien/R/
